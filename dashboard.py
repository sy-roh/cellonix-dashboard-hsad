...
def check_password():
    ...
    def password_entered():
        if st.session_state["password"] == "cellonix2026!": # 원하는 비밀번호로 변경
...
if not check_password():
    st.stop() # 비밀번호가 틀리면 아래 대시보드 코드는 아예 실행되지 않음
# -----------------------------

# (여기서부터 기존 데이터 불러오기 및 그래프 코드 시작...)
import streamlit as st
import pandas as pd
...
# ---------------------------------------------------------
# 1. 페이지 및 기본 설정 & 초밀착(Compact) UI CSS
# ---------------------------------------------------------
st.set_page_config(page_title="셀로닉스 데이터 대시보드", layout="wide")
...
st.markdown("""
    ...
    /* 상단 타이틀 잘림 방지를 위해 padding-top을 넉넉하게 2.5rem으로 수정 */
    .block-container {
...
def load_data():
    file_cellti = "2601-2607 셀티아이 유입사용자_통계_데이터.xlsx"
    file_triad = "2601-2607 트리어드 유입사용자_통계_데이터.xlsx"
    ...
    df_main = pd.concat([df_cellti, df_triad], ignore_index=True)
...
# --- [수정 포인트] 기본 날짜를 최신 달의 1일 ~ 마지막 날로 설정 ---
min_date = df_all['날짜'].min().date()
max_date = df_all['날짜'].max().date()
default_start_date = max_date.replace(day=1) # 가장 최근 데이터 달의 1일
