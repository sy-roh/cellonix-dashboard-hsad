...
# =========================================================
# [순서 6] NEW: 매체별 증감 추이 및 전체 현황표
# =========================================================
st.markdown("#### 🔄 비교 기간 대비 매체 운영 현황")
st.caption("※ 설정된 기간과 직전 동일 기간을 비교합니다. (현재 데이터상 '예산' 항목 부재로 **총 유입수** 기준으로 규모 증감을 분석합니다.)")

# 현재 기간 vs 직전 기간 유입수 집계
...
# 두 데이터프레임 병합 (Outer Join으로 없어진 매체, 새로 생긴 매체 모두 포함)
...
df_compare['증감률(%)'] = df_compare.apply(lambda r: calc_rate(r['이번_유입수'], r['이전_유입수']), axis=1)

# 보기 좋게 컬럼 정리 및 정렬
df_compare = df_compare[['상태', '매체', '이전_유입수', '이번_유입수', '증감량', '증감률(%)']]
df_compare = df_compare.sort_values(by='이번_유입수', ascending=False)

# 소수점 및 숫자 포맷팅을 위해 DataFrame 표시 설정
st.dataframe(
    df_compare, 
