...
def load_data():
    ...
    # 주의: Google Sheet 탭을 "내용만 수정"하면 GID는 유지되지만,
    # 탭을 삭제한 뒤 새로 만들거나 새 탭으로 교체하면 GID가 변경됩니다.
    SHEET_GIDS = {
        "25년 하반기": 875817815,
        "26년 상반기": 490015133,
        "26년 하반기": 1268027742,
    ...
    # -----------------------------------------------------
    # 3개 기간 시트를 하나의 로그 데이터로 통합
    # -----------------------------------------------------
    total_frames = []
    invalid_date_count = 0
    sheet_rows_loaded = {}

    for sheet_name in data_sheet_gids:
        part = read_sheet_csv(sheet_name, dtype={"날짜": "string"})
        part.columns = [str(c).strip() for c in part.columns]

        if "날짜" not in part.columns:
            raise ValueError(f"'{sheet_name}' 시트에서 '날짜' 컬럼을 찾지 못했습니다.")

        raw_part_date = part["날짜"].copy()
        part["날짜"] = parse_date_series(part["날짜"])

        invalid_mask = raw_part_date.notna() & part["날짜"].isna()
        invalid_date_count += int(invalid_mask.sum())

        # 날짜가 없는 행은 기간 분석에서 제외
        part = part[part["날짜"].notna()].copy()
        sheet_rows_loaded[sheet_name] = len(part)
        total_frames.append(part)

    if not total_frames:
        raise ValueError("기간별 로그 시트에서 데이터를 하나도 읽지 못했습니다.")

    # source sheet 컬럼을 만들지 않습니다.
    # 따라서 대시보드에서는 어떤 시트에서 왔는지 구분하지 않고 날짜 기준으로만 동작합니다.
    df_total = pd.concat(total_frames, ignore_index=True, sort=False)
    df_total = df_total.sort_values("날짜", kind="stable").reset_index(drop=True)
    df_total["브랜드"] = "전체"

    # 여러 시트를 합치면 일부 시트의 숫자 컬럼이 문자열로 추론될 수 있으므로
    ...
    for col in metric_cols:
        if col not in df_total.columns:
            # 시트 구조가 바뀌어 컬럼 자체가 없다면 0으로 생성해 하단 시각화 오류 방지
            df_total[col] = 0
        else:
    ...
    df_campaign.loc[
        df_campaign["시작일"] == df_campaign["종료일"], "종료일"
    ] += pd.Timedelta(days=1)
    ...
    if not category_cols:
        raise ValueError("통합 로그 데이터에서 CATEGORY_1~CATEGORY_5 컬럼을 찾을 수 없습니다.")
    ...
    influencer_dfs = []
    str_cols = df_total.select_dtypes(include=["object", "string"]).columns
    ...
    for _, row in df_campaign[df_campaign["구분"] == "인플루언서"].iterrows():
        ...
        if inf_name in inf_keywords:
            try:
                ...
                is_valid_date = (df_total["날짜"] >= start_date) & (
                    df_total["날짜"] <= end_date
                )
            except Exception:
                is_valid_date = True

            has_keyword = df_total[str_cols].apply(
                lambda x: x.astype(str).str.contains(inf_keywords[inf_name])
            ).any(axis=1)
    ...
    df_main = pd.concat(
        [df_total, df_cellti, df_triad, df_other] + influencer_dfs,
        ignore_index=True,
    )
    gc.collect()
    ...
    df_main["총방문수"] = (
        df_main["신규방문_총 방문수"].fillna(0)
        + df_main["재방문_총 방문수"].fillna(0)
    )
    df_main["총회원가입"] = (
        df_main["신규방문_회원가입"].fillna(0)
        + df_main["재방문_회원가입"].fillna(0)
    )
    df_main["총구매수"] = (
        df_main["신규방문_신규구매_건수"].fillna(0)
        + df_main["신규방문_재구매_건수"].fillna(0)
        + df_main["재방문_신규구매_건수"].fillna(0)
        + df_main["재방문_재구매_건수"].fillna(0)
    )
...
if valid_dates.empty:
    st.error("통합 로그 데이터의 날짜를 하나도 읽지 못했습니다. '날짜' 열의 값/형식을 확인해 주세요.")
    st.stop()
...
start_date, end_date = (
    date_range if len(date_range) == 2 else (date_range[0], date_range[0])
)
...
df_current = df_filtered[
    (df_filtered["날짜"].dt.date >= start_date)
    & (df_filtered["날짜"].dt.date <= end_date)
]
